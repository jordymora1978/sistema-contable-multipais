        ...
        for i in range(0, len(cleaned_records), batch_size):
            ...
            try:
                ...
                if order_ids:
                    ...
                    existing_result = supabase.table('consolidated_orders').select('order_id').in_('order_id', order_ids).execute()
                    existing_ids = {r['order_id'] for r in existing_result.data}
                    ...
                    for record in batch:
                        if record.get('order_id') in existing_ids:
                    ...
                    if new_records:
                        try:
                            result = supabase.table('consolidated_orders').insert(new_records).execute()
                            total_inserted += len(new_records)
                            st.info(f"✅ {len(new_records)} registros nuevos insertados en lote {i//batch_size + 1}")
                        except Exception as insert_error:
                            st.error(f"Error insertando nuevos: {str(insert_error)}")
                    
                    # Actualizar los existentes uno por uno
                    if update_records:
                        updated = 0
                        for record in update_records:
                            try:
                                order_id = record.get('order_id')
                                # Remover el order_id del registro para la actualización
                                update_data = {k: v for k, v in record.items() if k != 'order_id'}
                                
                                supabase.table('consolidated_orders').update(update_data).eq('order_id', order_id).execute()
                                updated += 1
                            except Exception as update_error:
                                # Ignorar errores de actualización individual
                                pass
                        
                        total_inserted += updated
                        st.info(f"✅ {updated} registros actualizados en lote {i//batch_size + 1}")
                else:
                    # Si no hay order_ids, insertar todo
