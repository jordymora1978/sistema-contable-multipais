            ...
            if result.data:
                ...
                for col_date in ['date_updated', 'created_at']:
                    if col_date in df_trm.columns:
                        df_trm[col_date] = pd.to_datetime(df[col_date], errors='coerce', infer_datetime_format=True) # Corrected df to df_trm
        ...
        if drapify_file:
            try:
                with st.spinner("Procesando archivos... Esto puede tardar unos segundos."):
                    ...
                    df_processed['gss_logistica'] = gss_logistica_base * (df_processed['account_name'] == '8-faborgargo').astype(float) # Corrected to snake_case
                    df_processed['impuesto_gss'] = df_processed['impuesto_gss'] * (df_processed['account_name'] == '8-faborgargo').astype(float) # Corrected to snake_case
