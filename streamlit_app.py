                ...
                with st.spinner("Procesando archivos... Esto puede tardar unos segundos."):
                    ...
                    if anican_file:
                        try:
                            ...
                            if all(col in df_anican.columns for col in required_anican_cols):
                                ...
                                merged_anican = df_processed[['order_id']].merge(
                                    df_anican[['reference', 'total']].rename(
                                        columns={'reference': 'order_id', 'total': 'total_anican_merged_temp'}),
                                    on='order_id',
                                    how='left'
                                )
                                df_processed['total_anican'] = merged_anican['total_anican_merged_temp'].fillna(df_processed['total_anican'])
                    ...
                    if aditionals_file:
                        try:
                            ...
                            if all(col in df_aditionals.columns for col in required_aditionals_cols):
                                ...
                                # Merge y manejo de columnas
                                merged_aditionals = df_processed[['order_id']].merge(
                                    aditionals_grouped[['order_id', 'aditional_merged_temp']],
                                    on='order_id',
                                    how='left'
                                )
                                df_processed['aditional'] = merged_aditionals['aditional_merged_temp'].fillna(df_processed['aditional'])
                    ...
                    if cxp_file:
                        try:
                            ...
                            # Detectar si es CSV o Excel por MIME type o extensión
                            if cxp_file.name.endswith('.csv'):
                                df_cxp_raw = pd.read_csv(cxp_file)
                            else:
                                # Try reading as Excel, iterating through sheets and headers
                                    ...
                                    for header_row in range(5): # Probar encabezados en las primeras 5 filas (0-4)
                                        try:
                                            ...
                                            if ref_col and amt_col:
                                                ...
                                                st.info(f"✅ CXP: Encabezado detectado en hoja '{sheet_name}', fila {header_row}.")
                                                break 
                                        except Exception as e:
                                ...
                                if df_cxp_raw is None:
                                    ...
                                    # Fallback to empty df_cxp to prevent errors later
                                    df_cxp_raw = pd.DataFrame() 

                            if not df_cxp_raw.empty:
                                df_cxp_raw.columns = [to_snake_case(col) for col in df_cxp_raw.columns] # Ensure normalized
...
""", unsafe_allow_html=True)
Entiendo que los montos de Anican y Chile Express no se están adjuntando correctamente, y que sigues viendo "0 coincidencias" y errores relacionados con columnas de CXP. Esto indica un problema persistente en la fusión de los datos.

He realizado una revisión profunda de la lógica de procesamiento de archivos y las fusiones (`merge`) para hacerla más robusta. Los cambios clave son:

1.  **Lectura de Archivos más Robusta**: Ahora, el sistema detecta el tipo de archivo (Excel o CSV) basándose en la **extensión del archivo** subido. Esto asegura que `Logistics.xlsx - Sheet.csv` y `Aditionals.xlsx - Sheet.csv` se lean correctamente como CSV si ese es su formato real, y como Excel si son `.xlsx` o `.xls`.
2.  **Manejo Explícito de Columnas Fusionadas**: He modificado la forma en que las columnas de los archivos Anican y CXP se incorporan al DataFrame principal (`df_processed`).
    * Para Anican y Aditionals, ahora se usan nombres temporales durante el `merge` y luego se actualiza la columna final (`total_anican`, `aditional`) usando `.fillna(valor_existente)` si no hubo coincidencia, asegurando que la columna siempre exista y tenga un valor predeterminado de 0 si no se encuentra.
    * Para CXP, la lógica de fusión se ha reestructurado para ser aún más explícita, garantizando que las columnas `amt_due_cxp`, `arancel_cxp`, e `iva_cxp` siempre existan en `df_processed` y contengan `0.0` si no hay coincidencias con el archivo CXP. Esto debería resolver el `KeyError` de `amt_due_cxp`.
3.  **Depuración Mejorada**: Se han añadido más mensajes `DEBUG` que muestran los `head()`, `dtypes` y ejemplos de las columnas clave de los DataFrames en puntos críticos. Esto te permitirá ver exactamente qué datos se están cargando y qué tipo tienen las columnas antes y después de cada fusión, lo cual es vital para identificar discrepancias en los `order_id` o `reference` entre tus archivos.

**Para que esto funcione, por favor:**

1.  **Copia y pega el código completo** en tu archivo `streamlit_app.py`.
2.  **Vuelve a ejecutar la aplicación** y carga tus archivos.
3.  **Observa los mensajes `DEBUG` en la consola de Streamlit**: Presta atención a los `order_id` y `reference` de cada archivo. Si los valores no coinciden exactamente (ej. "12345" vs " 12345" o "12345" vs "12345.0"), los `merge` no funcionarán. Podrías necesitar limpiar esos identificadores en tus archivos originales.
...
def init_supabase():
    ...
    try:
        ...
        # Si estas líneas causan un KeyError, significa que st.secrets no está configurado.
        # En ese caso, se usarán las claves directamente codificadas (menos seguro para producción).
        url = st.secrets.get("supabase", {}).get("url", "[https://qzexuqkedukcwcyhrpza.supabase.co](https://qzexuqkedukcwcyhrpza.supabase.co)")
        key = st.secrets.get("supabase", {}).get("key", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF6ZXh1cWtlZHVrY3djeWhycHphIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTM3NDEzODcsImV4cCI6MjA2OTMxNzM4N30.T_lXTVGZCFGA5rjVWQNo3WphIE2YPaifxonHIGPMkI0") # Corrected key (example, replace with actual)
